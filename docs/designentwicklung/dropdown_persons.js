...
function BasicMenu(var1, par1="", par2="", par3="") {
	...
	c=$("<div class=\"form-group\" id="+obj[numbers[index]][1]+"><div id="+obj[numbers[index]][2]+"><h6 id="+obj[numbers[index]][3]+"></h6></div></div>")
	var data=obj[numbers[index]][0];
	var s = $("<select id="+obj[numbers[index]][5]+" class=\"form-control\" multiple/>");
	for(var val in data) {
	    $("<option/>", {value: data[val], text: data[val]}).appendTo(s);
	 }
	c.appendTo("#container")
    ...
    function DropdownMenu(var1, var2, par1)
	{
		...
		$(".form-group#"+obj[numbers[index]][1]).show()
		       });
	    $.each(alphabet_orig, function( index) {
...
var currentBeta="";
	...
	$( "<p style='display:inline; font-size:20px;'>Current Selection:<br><br></p>" ).appendTo("#header")
	...
	$( "<p id='ids' style='opacity:0.3; font-size:18px;'><br>ID:<br></p>" ).appendTo("#header")
	$( "<p id='originals' style='opacity:0.3; font-size:18px;'><br>Persons (original):<br></p>" ).appendTo("#header")

	$("#selectionresult").appendTo("#selectedvalues")
...
function getDropdownPersons()
{
	...
	$("#main_content_engl").appendTo("#main_container")
    ...
    // ID-Anzeige
    containerlistID=["#bsd21-container"]
    $("[id^=Number]").css("background-color","white")
    currentNumber="1-100";
    
    $("#main_content_id").on("click", "[id^=number]", function () {
	
	$("[id^=numbers]").css("color","black")
	$("[id^=numbers]").css("font-size","15px")
        currentNumber=$(this).attr("id").replace('numbers','');
	$("#"+$(this).attr("id")).css("color","#007bff")
	$("#"+$(this).attr("id")).css("font-size","18px")
	var data=[persons[3].ID[$(this).text()][0]];
	str=containerlistID.pop()
	$(str).hide()
	$(obj[$(this).text()][7]).show()
	$("#main_content_id").appendTo("#main_container")
	$("#main_content_id").show();
	
	$(obj[$(this).text()][7]).find(".dropdown-menu.dropdown-menu-right").css({"top":"0","right":"-20px","position":"absolute"}).show();
	
        containerlistID.push(obj[$(this).text()][7]) 
    ...
    currentBeta="B";
    containerlistorig=["#bsd45-container"]
	...
	$("#main_content_orig").appendTo("#main_container")
	...
	id_container.push("bsd"+String(index+i+2)+"-button")
    })
    ...
    $(".form-control").css({"border":"0px","font-size":"12px"})
		...
		$("#main_content_engl").appendTo("#main_content");
		$("#main_content_engl").show();
		...
		$(v).find(".dropdown-menu.dropdown-menu-right").show();
            ...
            if($.inArray($(this).attr("id"), id_container) != -1) {
		$("#main_content_engl").hide();
		$("#main_content_orig").hide();
		$("#main_content_id").appendTo("#main_content");
		$("#main_content_id").show();
		v="#"+$(this).attr("id")
		...
		if($.inArray($(this).attr("id"), name_container) != -1) {
		    ...
		    $("#main_content_engl").hide();
		    $("#main_content_id").hide();
		    $("#main_content_orig").appendTo("#main_content");
